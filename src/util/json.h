...
#define ____READ____ std::shared_lock<std::shared_mutex> lock(m_mutex)
#define ____WRITE____ std::unique_lock<std::shared_mutex> lock(m_mutex)
#else
#define ____READ____
#define ____WRITE____
#endif
...
namespace newobj
{
	...
	public:
		...
		bool parse(const nstring& value);
		void path(const nstring& path);
		void safe(bool safe);
		bool safe();

		static newobj::json from(const nstring& value);

		bool parse_file(const nstring& filepath);
		bool save(nstring filepath = "");
		...
		void erase(const nstring& key);
		void erase(uint32 index);
		std::vector<nstring> keys() const;

		const nstring& to_string(bool format = false) const;
		newobj::json& operator[](const nstring name);
		const newobj::json& operator[](const nstring name) const;
		newobj::json& operator[](const uint32 idx);
		const newobj::json& operator[](const uint32 idx) const;
		...
		void operator=(const std::vector<nstring>& value);
		...
		void operator=(const std::vector<newobj::json>& value);
#ifndef MSVC_2010
		void operator=(std::initializer_list<newobj::json> value);
#endif
		void operator=(std::map<const char*, newobj::json> value);
		...
		void operator=(nstring value);
		void operator=(const bool value);
		void operator=(double value);
		void operator=(float value);
		void operator=(const char* value);
		void operator=(newobj::json::type value);
		void operator=(const newobj::json& value);

		void push_back(uint32 value);
		void push_back(int32 value);
		void push_back(uint64 value);
		void push_back(int64 value);
		void push_back(nstring value);
		void push_back(const bool value);
		void push_back(double value);
		void push_back(float value);
		void push_back(const char* value);
		void push_back(const newobj::json& value);
		void clear();
...
#define CASE_T(TYPE) std::is_same<TYPE,T_TO>::value
			if (CASE_T(std::vector<nstring>))
			{
				std::vector<nstring> result;
				if (is_array() == false)
					RETURN_T(result);
				for (uint32 i = 0;i < size();i++)
					result.push_back(this->operator[](i).to<nstring>());
				RETURN_T(result);
			}
			else if (CASE_T(std::vector<newobj::json>))
			{
				std::vector<newobj::json> result;
				if (is_array() == false)
					RETURN_T(result);
				for (uint32 i = 0;i < size();i++)
			...
			else if (CASE_T(int) || CASE_T(unsigned int) || CASE_T(long))
			{
				...
				if (m_number)
					...
					result = m_value_string.to_int32();
				RETURN_T(result);
			}
            else if (CASE_T(short) || CASE_T(unsigned short))
            {
                ...
                if (m_number)
                    ...
                    result = m_value_string.to_short();
                RETURN_T(result);
            }
			else if (CASE_T(nstring))
			{
			...
			else if (CASE_T(long long) || CASE_T(unsigned long long))
			{
				...
				if (m_number)
					...
					result = m_value_string.to_uint64();
				RETURN_T(result);
            }
	...
	private:
		nstring m_path;
	private:
		std::map<nstring, json*> m_objs;
		std::vector<json*> m_arrs;
		//std::shared_mutex m_mutex;
		
		nstring m_name;
		
		nstring m_value_string;
		bool m_safe;
